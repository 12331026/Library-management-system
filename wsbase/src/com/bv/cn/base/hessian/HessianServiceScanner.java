    ...
    private final class Scanner extends ClassPathBeanDefinitionScanner {
        ...
        protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
            ...
            for (String basePackage : basePackages) {
                ...
                for (BeanDefinition candidate : candidates) {
                    ...
                    for(String iface: interfaces) {
                    	logger.info("fount interface of "+originalBeanName + ":" + iface);
                    }
                    ...
                    BeanDefinitionHolder definitionHolder = new BeanDefinitionHolder(candidate,
                          "/" +   beanName );
                    logger.info("found bean:[" + originalBeanName + "]/" + beanName);
            ...
            if (beanDefinitions.isEmpty()) {
                logger.debug("not service be scaned");
            } else {
                for (BeanDefinitionHolder holder : beanDefinitions) {
                    AnnotatedBeanDefinition definition = (AnnotatedBeanDefinition) holder
                            .getBeanDefinition();
                    logger.debug(holder.getBeanName());

                    logger.debug(definition.getMetadata().getAnnotationTypes());
                }
            }
